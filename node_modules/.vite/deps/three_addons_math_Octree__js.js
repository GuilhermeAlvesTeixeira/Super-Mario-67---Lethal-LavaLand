import { Bs as Vector3, Dr as Line3, U as Box3, _r as Layers, _s as Triangle, ra as Plane, zo as Sphere } from "./three.module-Dhi4sXJn.js";
import { Capsule } from "./three_addons_math_Capsule__js.js";
//#region node_modules/three/examples/jsm/math/Octree.js
var _v1 = new Vector3();
var _v2 = new Vector3();
...
function lineToLineClosestPoints(line1, line2, target1 = null, target2 = null) {
	...
	if (target1) target1.copy(r).multiplyScalar(t1).add(line1.start);
	if (target2) target2.copy(s).multiplyScalar(t2).add(line2.start);
}
/**
...
var Octree = class Octree {
	...
	*/
	split(level) {
		if (!this.box) return;
		const subTrees = [];
		const halfsize = _v2.copy(this.box.max).sub(this.box.min).multiplyScalar(.5);
		...
		let triangle;
		while (triangle = this.triangles.pop()) for (let i = 0; i < subTrees.length; i++) if (subTrees[i].box.intersectsTriangle(triangle)) subTrees[i].triangles.push(triangle);
		for (let i = 0; i < subTrees.length; i++) {
			const len = subTrees[i].triangles.length;
			if (len > this.trianglesPerLeaf && level < this.maxLevel) subTrees[i].split(level + 1);
	...
	getRayTriangles(ray, triangles) {
		for (let i = 0; i < this.subTrees.length; i++) {
			...
			if (subTree.triangles.length > 0) {
				for (let j = 0; j < subTree.triangles.length; j++) if (triangles.indexOf(subTree.triangles[j]) === -1) triangles.push(subTree.triangles[j]);
	...
	*/
	triangleCapsuleIntersect(capsule, triangle) {
		...
		if (triangle.containsPoint(intersectPoint)) return {
		...
		for (let i = 0; i < lines.length; i++) {
			lineToLineClosestPoints(line1, _line2.set(lines[i][0], lines[i][1]), _point1, _point2);
			if (_point1.distanceToSquared(_point2) < r2) return {
	...
	*/
	triangleBoxIntersect(box, triangle) {
		if (Math.max(triangle.a.x, triangle.b.x, triangle.c.x) < box.min.x || Math.min(triangle.a.x, triangle.b.x, triangle.c.x) > box.max.x || Math.max(triangle.a.y, triangle.b.y, triangle.c.y) < box.min.y || Math.min(triangle.a.y, triangle.b.y, triangle.c.y) > box.max.y || Math.max(triangle.a.z, triangle.b.z, triangle.c.z) < box.min.z || Math.min(triangle.a.z, triangle.b.z, triangle.c.z) > box.max.z) return false;
	...
	*/
	triangleSphereIntersect(sphere, triangle) {
		...
		if (triangle.containsPoint(sphere.center)) return {
			normal: _plane.normal.clone(),
	...
	*/
	getBoxTriangles(box, triangles) {
		for (let i = 0; i < this.subTrees.length; i++) {
			...
			if (subTree.triangles.length > 0) {
				for (let j = 0; j < subTree.triangles.length; j++) if (triangles.indexOf(subTree.triangles[j]) === -1) triangles.push(subTree.triangles[j]);
			} else subTree.getBoxTriangles(box, triangles);
		}
	}
	/**
	...
	*/
	boxIntersect(box) {
		...
		for (let i = 0; i < triangles.length; i++) if (result = this.triangleBoxIntersect(_box, triangles[i])) {
			hit = true;
	...
	*/
	sphereIntersect(sphere) {
		...
		for (let i = 0; i < triangles.length; i++) if (result = this.triangleSphereIntersect(_sphere, triangles[i])) {
			hit = true;
	...
	*/
	capsuleIntersect(capsule) {
		...
		for (let i = 0; i < triangles.length; i++) if (result = this.triangleCapsuleIntersect(_capsule, triangles[i])) {
			hit = true;
